...
function renderPending(orders){
  ...
  tb.innerHTML = orders.map(function(o){
    ...
    return '<tr class="'+rc+'">'
      +'<td><a class="order-num" href="/crm/order/'+o.id+'">'+o.number+'</a></td>'
      +'<td>'+o.client+'</td>'
      +'<td><b>'+fmt(o.total)+' ₽</b></td>'
      +'<td><small>'+(SL[o.status]||o.status)+'</small></td>'
      +'<td>'+ap+'</td>'
      +'<td><small class="text-muted">'+fmtDt(o.created_at)+'</small></td>'
      +'<td><a href="/crm/order/'+o.id+'" class="btn btn-xs btn-outline-primary" style="font-size:12px;padding:2px 10px">→</a></td>'
      +'</tr>';
  }).join('');
}
